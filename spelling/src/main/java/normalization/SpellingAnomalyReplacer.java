			...
			if (anomaly.getSuggestions() != null) {
				...
				for (int i = 0; i < suggestions.size(); i++) {
					...
					if (cost < minCost) {
						...
						bestReplacements.put(suggestedAction.getReplacement(),
								suggestedAction.getMethodThatGeneratedThisSuggestion());
				...
				if (replacement != null) {
					...
					System.out.println("Replacing anomaly:\t'" + anomaly.getCoveredText() + "'\twith\t'" + replacement
							+ "' (cost: " + minCost + ").");
